# version description: global-local learning, instance-memory
...
class ReIDBaseModel(nn.Module):
    ...
    def __init__(
        ...
        # (Optional) Stronger baseline: add local classifiers for local feature parts
        if self.num_parts > 1:
            self.local_classifiers = nn.ModuleList()
            self.local_heads = nn.ModuleList()
            for i in range(self.num_parts):
                # (Option 1:) follow the original code, both global and local branches
                #     share a same embedding layer self.head

                # (Option 2:) reduce the channels of local embedding feature
                #     but notes that the label_generator splits output features equally
                # embed_feat_local = embed_feat // self.num_parts if (
                #         embed_feat > 0 and self.num_parts > 0
                # ) else 0

                # (Option 3:) set the channels of local embedding feature equal to
                #     that of global embedding feature
                # embed_feat_local = embed_feat
                # self.local_heads.append(
                #     build_embedding_layer(self.backbone.num_features, embed_feat_local, dropout)
                # )

                # (Option 4:) employ local embedding layer, which can be flexible with the global ones
                self.local_heads.append(
                    build_embedding_layer(self.backbone.num_features, loc_embed_feat, dropout)
                )

                local_classifier = nn.Linear(self.local_heads[i].num_features, num_classes, bias=False)
                init.normal_(local_classifier.weight, std=0.001)
    ...
    def initialize_centers(self, centers, labels):
        if self.num_classes > 0:
            ...
            if self.num_parts > 1:
                assert (
                    self.include_global
                ), "global feature centers and classifier must be included"
                self.classifier.weight.data[
                    labels.min().item() : labels.max().item() + 1
                ].copy_(centers[0].to(self.classifier.weight.device))
                for i in range(self.num_parts):
                    self.local_classifiers[i].weight.data[
                        labels.min().item(): labels.max().item() + 1
                    ].copy_(centers[i+1].to(self.local_classifiers[i].weight.device))
            else:
                self.classifier.weight.data[
                    labels.min().item() : labels.max().item() + 1
                ].copy_(centers[0].to(self.classifier.weight.device))
    ...
    def forward(self, x):
        ...
        if isinstance(out, list):
            ...
            assert (self.include_global), "global branch must be included"
            feat = []
            feat.append(self.head(out[0]))
            for i in range(self.num_parts):
                feat.append(self.local_heads[i](out[i+1]))
            # original code: no embedding layer, only BN and drop out
        ...
        if self.num_classes > 0:
            if isinstance(feat, list):
                # use global and local classifiers
                prob = []
                assert (
                    self.include_global
                ), "global classifier must be included"
                prob.append(self.classifier(feat[0]))
                for i in range(self.num_parts):
                    prob.append(self.local_classifiers[i](feat[i+1]))
                # original code: use only one classifier
...
def build_model(
    ...
    model = ReIDBaseModel(
        ...
        cfg.MODEL.dropout,
        cfg.MODEL.num_parts, # (Optional) for global-local feature extraction
        cfg.MODEL.include_global, # (Optional) for global-local feature extraction
        cfg.MODEL.loc_embed_feat, # (Optional) for local branch embedding layer
    ...
    if (num_domains > 1) and cfg.MODEL.dsbn:
        if cfg.TRAIN.val_dataset in list(cfg.TRAIN.datasets.keys()):
            ...
            warnings.warn(
                ...
                f"train sets, we use "
                f"{list(cfg.TRAIN.datasets.keys())[-1]}'s BN instead, "
...
# the original ReIDBaseModel code
# class ReIDBaseModel(nn.Module):
#     """
#     Base model for object re-ID, which contains
#     + one backbone, e.g. ResNet50
