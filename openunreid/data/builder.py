...
from .transformers import build_test_transformer, build_train_transformer, build_visualize_transformer
...
def build_visualize_test_dataloader(cfg, one_gpu=False, **kwargs):
    """
    Build visualized testing data loader
    """
    ...
    # build transformer
    test_transformer = build_test_transformer(cfg)
    visualize_test_transformer = build_visualize_transformer(cfg)

    # build individual datasets
    datasets, queries, galleries = [], [], []
    for dn in dataset_names:
        query_data = build_dataset(
            dn, data_root, "query", del_labels=False, transform=test_transformer, vis_transform=visualize_test_transformer
        )
        gallery_data = build_dataset(
            dn, data_root, "gallery", del_labels=False, transform=test_transformer, vis_transform=visualize_test_transformer
        )
        datasets.append(query_data + gallery_data)
        queries.append(query_data.data)
        galleries.append(gallery_data.data)
