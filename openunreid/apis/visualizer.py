...
# from .file_io import PathManager # copy from fast-reid repo: fast-reid/fastreid/utils/visualizer
...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            if (cnt + 1) % 5 == 0:
                ...
                query_img = query_info['vis_img']
                cam_id = query_info['cid']
                # query_name = query_info['path'].split('/')[-1]
                query_name = query_info['id']
                all_imgs.append(query_img)
                query_img = np.rollaxis(np.asarray(query_img.numpy()*255, dtype=np.uint8), 0, 3)
                plt.clf()
                ...
                for i in range(max_rank):
                    ...
                    gallery_img = gallery_info['vis_img']
                    cam_id = gallery_info['cid']
                    id = gallery_info['id']
                    all_imgs.append(gallery_img)
                    gallery_img = np.rollaxis(np.asarray(gallery_img*255, dtype=np.uint8), 0, 3)
                    if cmc[i] == 1:
                        label = 'true'
                    ...
                    ax.set_title(f'{self.sim[q_idx, sort_idx[i]]:.3f}/{id}/{label}/cam{cam_id}')
                    ax.axis("off")
                # if actmap:
                #     act_outputs = []
                #
                ...
                if vis_label:
                    ...
                    for i in range(max_rank):
                        ...
                        ax.add_patch(plt.Rectangle(xy=(0, 0), width=gallery_img.shape[1] - 1,
                                                   ...
                                                   fill=False, linewidth=5))
                        ax.imshow(gallery_img)
                        ax.set_title(f'{self.sim[q_idx, sort_idx[j]]:.3f}/{id}/cam{cam_id}')
                        ax.axis("off")
    ...
    def vis_rank_list(self, output, vis_label, num_vis=100, rank_sort="ascending", label_sort="ascending", max_rank=5,
        ...
        query_indices = query_indices[:num_vis*5]
