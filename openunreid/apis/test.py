...
@torch.no_grad()
def test_reid(
    cfg, model, data_loader, query, gallery, dataset_name=None, rank=None, visualize=False, **kwargs
):
    ...
    # if "num_parts" in cfg.MODEL:
    #     if cfg.MODEL.num_parts > 1:
    #         feat = features[0]
    #         num_splits = cfg.MODEL.num_parts
    #         if cfg.MODEL.include_global is True:
    #             num_splits += 1
    #         for i in range(num_splits - 1):
    #             feat = torch.cat((feat,features[i+1]), dim=-1)
    #         features = feat
    ...
    if rank == 0:
        ...
        cmc, map, all_AP = evaluate_rank(dist, q_pids, g_pids, q_cids, g_cids) # all_AP for visualizer
    else:
    ...
    if cfg.TEST.rerank:
        ...
        if rank == 0:
            ...
            cmc, map, all_AP = evaluate_rank(final_dist, q_pids, g_pids, q_cids, g_cids) # all_AP for visualizer
        else:
            cmc, map = np.empty(50), 0.0

    if visualize is True and rank == 0:
        visualizer = Visualizer(data_loader.dataset)
        dist_vis = final_dist if (cfg.TEST.rerank) else dist
        visualizer.get_model_output(all_AP, dist_vis, q_pids, g_pids, q_cids, g_cids)
        print("Saving rank list results ...")
        outputpath = '../logs/vis_rank_list/' + dataset_name
        visualizer.vis_rank_list(output=outputpath,
                                 vis_label=True,
                                 num_vis=100,
                                 max_rank=10,
                                 rank_sort="ascending",
                                 label_sort="ascending")   # [ascending, descending]
...
def val_reid(
    ...
    if rank == 0:
        ...
        cmc, map, _ = evaluate_rank(dist, pids, pids, cids, cids) # all_AP is ignored, val_reid not modified
    else:
        cmc, map = np.empty(50), 0.0
