...
# add soft losses for global and local branches
# with cross-domain mixup
...
from ..utils.dist_utils import get_dist_info, synchronize, broadcast_value
...
class BaseRunner(object):
    ...
    def __init__(
        ...
        # (Optional:) initialize beta distribution for cross-domain mixup
        mixup_alpha = self.cfg.DATA.TRAIN.mixup_alpha
        self.beta_distr = Beta(mixup_alpha, mixup_alpha)
        self.mixup_lmd = 0.9
    ...
    def update_labels(self):
        ...
        for idx in range(len(self.cfg.TRAIN.datasets)):
            if idx in self.cfg.TRAIN.unsup_dataset_indexes:
                ...
                # for both global-local feature extraction or original pipeline (global only)
    ...
    def train(self):
        ...
        for iter in range(self.cfg.TRAIN.iters):
            ...
            # update mixup lambda for current mini-batch
            # self.mixup_lmd = self.beta_distr.sample().item()
            if self.cfg.DATA.TRAIN.is_mixup:
                mixup_lmd = self.mixup_lmd
                if self._rank == 0:
                # only on cuda:0
                    mixup_lmd = self.beta_distr.sample().item()
                synchronize()
                # broadcast to other GPUs
                if self.world_size > 1:
                    mixup_lmd = broadcast_value(mixup_lmd, 0)
                self.mixup_lmd = mixup_lmd
                # print("Mixup lambda update: ", self.mixup_lmd)
